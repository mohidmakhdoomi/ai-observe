...
class Timeline:
    ...
    def run(self, tool: str, ctx) -> list[CheckResult]:
        ...
        # loud, named ToolUnusable — not a generic viewer failure. Passing the probe's
        # persisted `stderr_tail` via `.meta` lets ensure_tool_usable surface the reason
        # inline (the same mechanism the session path uses), while the full log stays on
        # disk under --keep-artifacts.
        ensure_tool_usable(tool, types.SimpleNamespace(
            returncode=report["returncode"],
            disk_events={"total": report["canonical_total"]},
            meta={"stderr_tail": report.get("stderr_tail", "")}))
